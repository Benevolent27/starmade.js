...
// To uninstall from local dependencies:  npm uninstall -save prompt-sync
function forceRequire(nodeRequire){
  try {
    ...
    try {
      ...
      // var installDeps = child.execSync('npm install');
  ...
  // Get the main js file from the package.js file..
  ...
  // console.log("Setting require to: " + theMainJSFile);
...
if (settings) { theSettings=settings; }
  ...
  // let thisModuleFolder=__dirname + "/node_modules/" + nodeRequire + "/";
  // let theMainJSFile=thisModuleFolder + require(thisModuleFolder + "package.json")["main"];

  // console.log("Setting require to: " + theMainJSFile + "(" + theSettings.toString() + ")");
  return require(theMainJSFile)(theSettings);
}
const prompt = forceRequireWithSettings("prompt-sync",{"sigint":true}); // https://www.npmjs.com/package/prompt-sync - This creates sync prompts and can have auto-complete capabilties.
...
// do testPrompt=parseInt(prompt("DO/WHILE Loop number Please! "));
...
function testStarMadeDirValue (path) {
  if (!path) { return path.join(__dirname, "starmade"); }
  if (!isInvalidPath(path)) { // If the path provided was valid
    if (fs.existsSync(path)){ // If the path exists, check to see if it is a file or named pipe.  IF so, we cannot use it.
      if (fs.statSync(path).isFile()) {
        console.log("ERROR: '" + path + "' already exists as a filename.  Please choose a different directory path!");
      } else if (fs.statSync(path).isFIFO()) {
        console.log("ERROR: '" + path + "' already exists as a named pipe.  Please choose a different directory path!");
      } else {
        return path // The path existed and it was not a file or named pipe so we should be able to use it.. unless it's a symlink to a file.. but I figure if someone is using symlinks to a file they should be smart enough to know not to try to use it as their starmade install folder..
      }
    }
...
try {
  ...
  console.log("'" + settingsFile + "' file not found! Let's set up your install!");
  settings = {};
}
...
if (!settings.hasOwnProperty('starMadeFolder')) {
  console.log("What StarMade folder should we install to?");
  ...
  if (!fs.existsSync(settings["starMadeFolder"])){
    console.log("The StarMade Install folder does not exist!");
    if (prompt("Press [ENTER] to create the folder.  Anything else will exit this script!")){
      console.log("Aborting settings change and exiting script!");
      process.exit(130);
    }
    try {
