...
const fs                = require('fs');
const installAndRequire = require("./installAndRequire.js");
...
const mySleep=require("./mySleep.js");
const {softSleep:sleep,sleepPromise}=mySleep;
const objectHelper = require("./objectHelper.js");
...
function deleteFile (fileToDelete,options){ // options can be:  {"quiet":true/false}  This will still display an error if the file exists but cannot be deleted for some reason.
  ...
  let resolvedFile = path.resolve(__dirname,fileToDelete);
