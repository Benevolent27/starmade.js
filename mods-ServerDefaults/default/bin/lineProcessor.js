// This is not done.  I think the ServerObj will need to be changed over to be a mod as well..
// There will need to be a routine to set up the settings for a new server, but then to one of the mods will actually need to start the server.  Off the top of my head, perhaps there can be a "starter.js" script in the mods folder.  This then creates the server object.
// module.exports={
//     processDataInput,
...
var serverObj={};
module.exports=getModuleExports; // Example of how to import this:  starNet=require("lineProcessor.js")(serverObj);
function getModuleExports(theServerObj){ // This is used when requiring in this script, to set the server
  ...
  throw new Error("This script MUST be given a serverObj, so can determine what information to display to the console or not!");
}
...
var {getSimpleTime,getSimpleDate}=miscHelpers;
...
if (testMatch(dataInput)) { // Check to see if the message fits any of the regex patterns
    ...
    if (serverObj.settings.showAllEvents == true) {
    ...
    if (serverObj.settings.enumerateEventArguments == true){
    ...
    serverObj.event.emit('playerMessage',new serverObj.objects.MessageObj(sender,receiver,receiverType,message));
    ...
    if (/PlS\[.*/.test(theArguments[5].toString())){
        ...
        if (serverObj.settings["announceSpawnsToMainChat"] == "true") {
        ...
        serverObj.event.emit('playerSpawn',playerObj,playerSMNameObj);
        }
    }
    ...
    if (theArguments[0] == "[BLUEPRINT][BUY]"){ // New Ship spawn from blueprint
        ...
        if (err){
            ...
            serverObj.event.emit('blueprintSpawn',spawnType,playerObj,blueprintObj,entityObj,sectorObj,factionObj); // playerObj will be undefined if the blueprint was spawned by admin or mass spawned
        }
        });
    ...
    if (excerptArray){
        ...
        serverObj.event.emit('playerSectorChange',player,startingCoords,endingCoords,playerSMName); // playerObj will be undefined if the blueprint was spawned by admin or mass spawned
    }
    ...
    if (theArguments[1] == "Added"){
        ...
        serverObj.event.emit('playerFactionJoin',playerObj,factionObj,factionNameString);
    }
    ...
    if (theArguments[1] == "removing"){
        ...
        let factionObj=new serverObj.objects.FactionObj(factionID);
        serverObj.event.emit('playerFactionLeave',nameObj,factionObj,factionName);
    }
    ...
    if ((/connected: true$/).test(dataInput)){
        ...
        serverObj.event.emit('playerConnect',playerObj,playerSmNameObj);
    }
    ...
    let playerObj=new serverObj.objects.PlayerObj(playerName);
    let playerSmNameObj=new serverObj.objects.SMNameObj(playerSMName);
    serverObj.event.emit('playerDisconnect',playerObj,playerSmNameObj);
    ...
    return entityObj.exists("",function(err,result){
        ...
        return serverObj.event.emit('entityOverheat',entityObj,sectorObj,playerObj,playerSMNameObj);
    ...
    if ((/STOPPED OVERHEATING$/).test(dataInput)){
        ...
        return getUIDfromName(theName,"",function(err,theUID){
            ...
            return serverObj.event.emit("entityOverheatStopped",theEntityObj);
            }
            return false;
        })
...
if (testMatchServerLog(dataInput)) { // Check to see if the message fits any of the regex patterns
    ...
    if (shipName){
        ...
        if (err){
            ...
            serverObj.event.emit('shipSpawn',playerObj,entityObj);
        }
        });
    } else {
        ...
        return getUIDfromName(baseName,"",function(err,result){
            ...
            let entityObj=new serverObj.objects.EntityObj(result);
            entityObj.spawnTime=Math.floor((new Date()).getTime() / 1000);
            serverObj.event.emit('baseSpawn',playerObj,entityObj);
            }
        });
        }
    }
    ...
    if (person){
        ...
        if (deathType == "suicide"){
            serverObj.event.emit('playerDeath',personObj,deathType);
            } else if (deathType == "person"){
            console.debug(`${killer} killed ${person}.`);
            serverObj.event.emit('playerDeath',personObj,deathType,"","",killerObj);
            ...
            return getFactionObjFromName(serverObj,responsibleFaction,"",function(err,responsibleFactionObj){
                ...
                serverObj.event.emit('playerDeath',personObj,deathType,responsibleEntityObj,responsibleFactionObj,killerObj);
                }
            });
            } else {
            serverObj.event.emit('playerDeath',personObj,deathType,responsibleEntityObj,"",killerObj);
            }
            ...
            serverObj.event.emit('playerDeath',personObj,deathType,responsibleEntityObj); // responsibleEntityObj will be undefined if no EntityObj was given.  //TODO: This needs to be tested.
            ...
            serverObj.event.emit('playerDeath',personObj,deathType,responsibleEntityObj);
