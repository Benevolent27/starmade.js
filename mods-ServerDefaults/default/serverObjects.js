...
global.event.on("start",function(serverPath){ // This event only happens AFTER the serverObj has been created
    if (serverPath === myServerPath){
        ...
        server.regConstructor(BotObj);
        server.regConstructor(MessageObj);
        server.regConstructor(ChannelObj);
        server.regConstructor(IPObj);
        server.regConstructor(SMNameObj);
        server.regConstructor(PlayerObj);
        server.regConstructor(SystemObj);
        server.regConstructor(BlueprintObj);
        server.regConstructor(FactionObj);
        server.regConstructor(LocationObj);
        server.regConstructor(SectorObj);
        server.regConstructor(CoordsObj);
        server.regConstructor(CreatureObj);
        server.regConstructor(EntityObj);
        }
    }
});
...
function BotObj(name){ // cb/promises/squishy compliant 
    ...
    this.starMadeFolder=server.starMadeFolder; // This is needed to recreate the server object if needed after squishing the object
    ...
    this.serverMsg=function(msgString,options,cb){ // This expects the message to send either as a string or an object that can be converted to a string
        ...
        var theMessage=toStringIfPossible(msgString); // This allows certain objects that can be converted to strings to be used, such as matches or other objects
        if (typeof theMessage == "string"){
            return server.msg("[" + this.name + "]: " + theMessage,options,cb); // options are forwarded to server.msg
        } else {
...
function PlayerObj(name){ // cb/promises/squish compliant // "Player" must be a string and can be just the player's nickname or their full UID
    ...
    if (typeof thePlayer == "string"){
        ...
        if (typeof cb == "function"){
            ...
            return server.bot.msg(self.name,messageToSend,options,cb); // This should throw an error if there is a problem connecting to the server
        } else {
        ...
        if (typeof cb == "function"){
            ...
            if (fast){
                ...
                return starNetVerified(server,changeSectorCommand,options,function(err,result){ // TODO: Test this.  I don't know if I should be returning this or just running it?
                if (err){
                    return cb(err,result);
                } else {
        ...
        if (typeof cb == "function"){
            ...
            if (fast){
                ...
                return starNetVerified(server,teleportToCommand,options,function(err,result){
                if (err){
                    return cb(err,result);
                } else {
        ...
        if (typeof cb == "function"){
            var returnObj={};
            return starNetVerified(server,"/player_info " + self.name,options,function(err,result){
            if (err){
                return cb(err,result);
            }
        ...
        // RETURN: [SERVER, END; Admin command execution ended, 0]
        if (typeof cb=="function"){
            return starNetVerified(server,"/entity_info_by_player_uid " + self.name,options,function(err,result){
            if (err){
        ...
        if (typeof cb=="function"){
            ...
            return starNetVerified(server,"/player_info " + this.name,options,function(err,result){
            if (err){
        ...
        if (typeof cb=="function"){
            ...
            return starNetVerified(server,commandToUse + self.name,options,function(err,result){
            if (err){
        ...
        if (typeof cb=="function"){
            ...
            return starNetVerified(server,"/list_blueprints_by_owner " + self.name,options,function(err,result){
            if (err){
                return cb(err,result);
            }
...
function SystemObj(x,y,z){ // cb/promises/squish compliant
    ...
    this.typeNumber=function(options,cb){
        if (typeof cb == "function"){
        return simpleSqlQuery(server,`SELECT TYPE FROM PUBLIC.SYSTEMS WHERE X=${self.x} AND Y=${self.y} AND Z=${self.z};`,options,function(err,result){
            if (err){
            return cb(err,result);
            }
            if (result.length > 0){
    ...
    this.claimEntity=function(options,cb){
        if (typeof cb == "function"){
        return simpleSqlQuery(server,`SELECT OWNER_UID FROM PUBLIC.SYSTEMS WHERE X=${self.x} AND Y=${self.y} AND Z=${self.z};`,options,function(err,result){
            if (err){
            return cb(err,result);
            }
            if (result.length > 0){
    ...
    this.claimSector=function(options,cb){
        if (typeof cb == "function"){
        return simpleSqlQuery(server,`SELECT OWNER_X,OWNER_Y,OWNER_Z FROM PUBLIC.SYSTEMS WHERE X=${self.x} AND Y=${self.y} AND Z=${self.z};`,options,function(err,result){
            if (err){
            return cb(err,result);
            }
            if (result.length > 0){
    ...
    this.claimFaction=function(options,cb){
        if (typeof cb == "function"){
        return simpleSqlQuery(server,`SELECT OWNER_FACTION FROM PUBLIC.SYSTEMS WHERE X=${self.x} AND Y=${self.y} AND Z=${self.z};`,options,function(err,result){
            if (err){
            return cb(err,result);
            }
            if (result.length > 0){
...
function BlueprintObj(name){  // cb/promises/squish compliant
    ...
    this.info = function(options,cb){
        if (typeof cb=="function"){
        return starNetVerified(server,"/blueprint_info '" + self.name + "'",options,function(err,result){
            var outputObj={};
            if (err){
            return cb(err,result);
            }
...
function FactionObj(number){  // cb/promises/squish compliant
    ...
    this.name=function(options,cb){
        if (typeof cb=="function"){
        return starNetVerified(server,"/faction_list",options,function(err,result){
            if (err){
            return cb(err,result);
            }
    ...
    this.listMembers=function(options,cb){
        ...
        return starNetVerified(server,"/faction_list_members " + self.number,options,function(err,result){
            if (err){
            return cb(err,result);
            }
    ...
    this.getFactionPoints=function(options,cb){
        if (typeof cb == "function"){
        return starNetVerified(server,"/faction_point_get " + self.number,options,function(err,result){
            if (err){
            return cb(err,result);
            }
...
function SectorObj(x,y,z){ // cb/promises/squish compliant
    ...
    if (typeof self.x == "number" && typeof self.y == "number" && typeof self.z == "number"){
        ...
        if (typeof cb == "function"){
            return starNetVerified(server,"/sector_info " + self.coords.toString(),options,function(err,result){
            if (err){
                return cb(err,result);
            }
        ...
        if (typeof cb=="function"){
            ...
            if (typeof theNumToUse=="number"){
            return starNetVerified(server,"/force_save",options,async function(err){ // Not sure if this needs to be async
                if (err){
                return cb(err,null);
                }
    ...
    if (fullUIDToUse){
        ...
        self.changeSector=function(sector,options,cb){ // sector can be a LocationObj, SectorObj, CoordsObj, or other input that can be translated to a CoordsObj.
            ...
            if (fast){
                ...
                return starNetVerified(server,changeSectorCommand,options,function(err,result){
                if (err){
                    return cb(err,result);
                }
                // Success: RETURN: [SERVER, [ADMIN COMMAND] [SUCCESS] changed sector for Benevolent27 to (1000, 1000, 1000), 0]
        ...
        if (typeof cb == "function"){
            return simpleSqlQuery(server,`SELECT TYPE FROM PUBLIC.ENTITIES WHERE UID='${self.UID}';`,options,function(err,result){
            if (err){
                return cb(err,result);
            }
            if (result.length > 0){
        ...
        if (typeof cb == "function"){
            return simpleSqlQuery(server,`SELECT TOUCHED FROM PUBLIC.ENTITIES WHERE UID='${self.UID}';`,options,function(err,result){
            if (err){
                return cb(err,result);
            }
            if (result.length > 0){
        ...
        if (typeof cb == "function"){
            return simpleSqlQuery(server,`SELECT DOCKED_TO FROM PUBLIC.ENTITIES WHERE UID='${self.UID}';`,options,function(err,result){
            if (err){
                return cb(err,result);
            }
            if (result.length > 0){
                ...
                return simpleSqlQuery(server,`SELECT UID,TYPE FROM PUBLIC.ENTITIES WHERE ID='${dockedTo}'`,options,function(err,result){
                    if (err){
                    return cb(err,result);
                    }
        ...
        // Returns undefined if not in the world file yet.
        if (typeof cb == "function"){
            return simpleSqlQuery(server,`SELECT DOCKED_ROOT FROM PUBLIC.ENTITIES WHERE UID='${self.UID}';`,options,function(err,result){
            if (err){
                return cb(err,result);
            }
            if (result.length > 0){
...
if (ipAddress){
    ...
    if (typeof minutesNum == "number"){
        ...
        return starNetVerified(server,"/whitelist_ip_temp " + ipToUse + " " + minutesNum,options,function(err,result){
        if (err){
    ...
    return starNetVerified(server,"/whitelist_ip " + ipToUse,options,function(err,result){
        if (err){
...
if (ipAddress){
    ...
    if (typeof minutesNum == "number"){
        ...
        return starNetVerified(server,"/ban_ip_temp " + ipToUse + " " + minutesNum,options,function(err,result){
        if (err){
    ...
    return starNetVerified(server,"/ban_ip " + ipToUse,options,function(err,result){
        if (err){
...
if (ipAddress){
    ...
    return starNetVerified(server,"/unban_ip " + ipToUse,options,function(err,result){
    if (err){
        console.error();
        return cb(err,result);
    }
...
if (typeof theChmodString == "string"){
    ...
    return starNetVerified(server,theCommand,options,function(err,result){
        if (err){
        return cb(err,result);
        }
...
if (typeof theCoords == "string"){
    ...
    return starNetVerified(server,"/sector_info " + theCoords,options,function(err,shipListResults){
        if (err){
...
// takes input from banlist or whitelist, producing an array, running a function on each one.
return starNetVerifiedCB(server,command,options,function(err,result){
    if (err){
    return cb(err,null);
    } else {
...
if (typeof cb=="function"){
    ...
    try {
    var result=starNetVerified(server,theCommand,options);
...
if (fast==true){ // Perform file read style
    ...
    return starNetVerified(server,"/list_admins",options,function(err,result){
    if (err){
...
if (testIfInput(input)){
    try {
    theResult=server.spawn.stdin.write(input + "\n");
    } catch (err){
    theErr=err;
    }
    if (typeof cb=="function"){
...
if (typeof cb == "function"){
    return starNetVerified(server,"/player_get_spawn " + player,options,function(err,result){
    if (err){
        return cb(err,result);
    } else {
...
if (typeof cb == "function"){
    ...
    return starNetVerified(server,theCommandToUse,options,function(err,msgResult){
        if (err){
