...
function ServerObj(starMadeFolder,options){ // starMadeFolder is optional.  If given it will load existing settings or create new ones if needed. // cb/promises/squish compliant 
    ...
    if (typeof starMadeFolder == "string"){ // Load up settings if they exist
      this.starMadeFolder=path.resolve(global["mainFolder"],starMadeFolder); // This only adds to the mainFolder if a full path was not given as 'starMadeFolder', which shouldn't happen normally.
      if (global.settings.servers.hasOwnProperty(self.starMadeFolder)){
        this.settings=global.settings.servers[self.starMadeFolder]; // Use the settings from the settings file
      }
    ...
    if (global["servers"].hasOwnProperty(self.starMadeFolder)){ // Check to see if this serverObj has already been created, returning that object if so.
      console.error("Server already initialized!  Ignoring any settings you may have set and using the existing server object!");
      return global["servers"][self.starMadeFolder]; // I have no idea if this will work or not
    }
      ...
      if (typeof theFunction == "function"){
        if (theFunction.hasOwnProperty("name")){
          ...
          if (firstLetter === letterTest){
            throw new Error("Unable to register constructor! Constructor functions should have an uppercase first letter! '" + theFunction.name + "' does not have an uppercase first letter! -- Source Server: " + self.starMadeFolder);
          } else {
            self.objects[theFunction.name]=theFunction;
            console.log("Registered new Constructor, '" + theFunction.name + "', for server: " + self.starMadeFolder); // This does not make it a constructor.
            return true;
          }
        }
        throw new Error("Unable to register unnamed constructor!  Please only attempt to register VALID constructors!  Server: " + self.starMadeFolder);
      }
      return false;
    }
    this.deregConstructor=function (theFunction){
      ...
      if (typeof theFunction == "function"){
        if (theFunction.hasOwnProperty("name")){
          ...
          throw new Error("Invalid input given to deregConstructor!  Expects a named function or string! Server: " + self.starMadeFolder);
        }
    ...
    if (self.settings.hasOwnProperty("lockPIDs")){
      ...
      if (self.settings.lockPIDs.length>0 && ignoreLockFile == false){
        ...
        var settingsBackupFile=path.join(self.starMadeFolder,"settingsBackup.json");
    ...
    ensureFolderExists(self.starMadeFolder);
    this.starMadeInstallFolder=path.join(self.starMadeFolder,"StarMade");
    ensureFolderExists(self.starMadeInstallFolder);
    this.logsFolder=path.join(self.starMadeFolder,"logs");
    ...
    this.log=function(logMsg){ // Writes to a log file with the current date into the /log subfolder
      ...
      if (typeof logMsg=="string"){
        ...
        var errorMsg="Invalid input given to serverObj.log function!  Expects a string!  Server: " + self.starMadeFolder;
    ...
    if (!existsAndIsDirectory(self.modsFolder)){ //check to see if a mods folder exists, and if not, copy the mods from the root folder over.
      fsExtra.copy(global["modsFolder"],self.starMadeFolder,function(err){
        if (err){
          throw err;
        }
    ...
    this.install=function(options,cb){
      if (typeof cb == "function"){
        ...
        try {
          smInstallHelpers.spawnStarMadeInstallTo(self.starMadeFolder,global["starMadeInstallerFilePath"]); // Does not create config files upon install.
          smInstallHelpers.verifyInstall(self.starMadeFolder); // Creates config files if they don't exist
    ...
    if (!existsAndIsFile(self.serverCfgFilePath)){ // Right now we are only using the server.cfg file to check if it was installed or not.
      self.install("",function(err){
        ...
        console.log("Install successful to folder: " + self.starMadeFolder);
      });
    }
    ...
    this.start=function(options,cb){
      if (typeof cb == "function"){
        ...
        global["servers"][self.starMadeFolder]=self.spawn;
